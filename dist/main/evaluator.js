...
function resolveName(name, state) {
    return state.aliases.get(name) ?? name;
}
function stringifyValue(value) {
    return typeof value === 'string' ? value : String(value);
}
function interpolateString(text, state) {
    ...
    while (index < text.length) {
        ...
        const lexer = new lexer_1.Lexer(expressionText);
...
function evaluate(expr) {
    return evaluateWithState(expr, { variables: new Map(), aliases: new Map() });
}
